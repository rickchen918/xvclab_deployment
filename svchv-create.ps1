...
$i=1
while($i -le $vm_count) {
	New-VM -VMHost $vmhost -Name svc-esxi$i -Datastore $ds -DiskMB $diskspace -MemoryMB $memory -NumCpu $numcpu -GuestID $guestid -NetworkName $network1,$network2
	$vm=Get-VM | ? {$_.name -match "svc-esxi$i"}
	$nicnum=Get-NetworkAdapter $vm |  ?{$_.name -match "Network adapter 1"}
    	$mac_addr="00:50:56:00:00:1$i"
