...
new-datacenter -location datacenters -name svc-dc

# create infra clusetrs
new-cluster -location svc-dc -name svc-cluster 

# register hosts to vc
add-vmhost $esx1 -Location svc-dc -User root -Password nicira123 -Force
add-vmhost $esx2 -Location svc-dc -User root -Password nicira123 -Force

# move host into cluster
move-vmhost $esx1 -destination svc-cluster
move-vmhost $esx2 -destination svc-cluster
...
# create new vds 
new-vdswitch -name svc-nsx -location svc-dc -NumUplinkPorts 2

# attach host into vds 
Add-VDSwitchVMHost -VDSwitch svc-nsx -VMHost $esx1
Add-VDSwitchVMHost -VDSwitch svc-nsx -VMHost $esx2

# attach vmnic1 to vds
$vmhostnetworkadpater=get-vmhost $esx1 |get-vmhostnetworkadapter -physical -name vmnic1
get-vdswitch svc-nsx | add-vdswitchphysicalnetworkadapter -vmhostnetworkadapter $vmhostnetworkadpater -confirm:$false
